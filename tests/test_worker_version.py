...
def test_heartbeat_version_surfaces_on_workers(client):
    _heartbeat(client, "broker-host", version="1.2.3")
    assert _worker(client, "broker-host")["version"] == "1.2.3"
...
def test_metrics_expose_worker_version_series(client):
    ...
    _heartbeat(client, "broker-host", version="1.2.3")
    _heartbeat(client, "worker-b", version="1.2.2")
    _heartbeat(client, "ancient")

    body = client.get("/metrics").text
    assert 'jobd_worker_version_info{host="broker-host",version="1.2.3"} 1.0' in body
    assert 'jobd_worker_version_info{host="worker-b",version="1.2.2"} 1.0' in body
