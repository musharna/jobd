...
def test_heartbeat_version_surfaces_on_workers(client):
    _heartbeat(client, "broker-host", version="0.5.15")
    assert _worker(client, "broker-host")["version"] == "0.5.15"
...
def test_version_is_not_pinned_to_last_known_value(client):
    ...
    _heartbeat(client, "broker-host", version="0.5.15")
    assert _worker(client, "broker-host")["version"] == "0.5.15"
    _heartbeat(client, "broker-host")  # older build, reports nothing
    assert _worker(client, "broker-host")["version"] is None
...
def test_metrics_expose_worker_version_series(client):
    ...
    _heartbeat(client, "broker-host", version="0.5.15")
    _heartbeat(client, "worker-b", version="0.5.14")
    _heartbeat(client, "ancient")

    body = client.get("/metrics").text
    assert 'jobd_worker_version_info{host="broker-host",version="0.5.15"} 1.0' in body
    assert 'jobd_worker_version_info{host="worker-b",version="0.5.14"} 1.0' in body
