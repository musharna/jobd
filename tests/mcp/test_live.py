...
Hits the real jobd broker (`JOBD_URL` env, default `http://100.113.204.41:8765`)
...
green from masking schema drift again — see
`memory/jobd_mcp_schema_drift_2026-04-26.md` for the incident this covers.
...
JOBD_URL = os.environ.get("JOBD_URL", "http://100.113.204.41:8765")


def _broker_reachable() -> bool:
    try:
        httpx.get(f"{JOBD_URL}/workers", timeout=3.0).raise_for_status()
        return True
    except Exception:
        return False
