...
hang-guard on all 32, including project-x: the project whose zombie created it.
...
def test_a_registered_project_still_inherits_the_hang_guard():
    ...
    projects = _fleet(**{"project-x": ProjectEntry(priority=70)})
    d = resolve_project_defaults(projects, "project-x")
...
def test_the_overlay_path_that_actually_broke():
    ...
    projects = apply_project_overrides(_fleet(), {"project-x": 70, "project-y": 65})
    for name in ("project-x", "project-y"):
...
@pytest.mark.parametrize("project", ["project-x", "never-registered"])
