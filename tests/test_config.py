...
def test_load_projects(sample_projects_yaml):
    ...
    assert projects["phelipanche"] == 80
    assert projects["orchid-sdxl"] == 55
    assert projects["_default"] == 40
...
def test_resolve_priority_known_project(sample_projects_yaml):
    projects = load_projects(sample_projects_yaml)
    assert resolve_priority(projects, "phelipanche", delta=0) == 80
    assert resolve_priority(projects, "phelipanche", delta=5) == 85
    assert resolve_priority(projects, "phelipanche", delta=-15) == 65
...
def test_resolve_priority_clamps(sample_projects_yaml):
    projects = load_projects(sample_projects_yaml)
    assert resolve_priority(projects, "phelipanche", delta=50) == 100
    assert resolve_priority(projects, "phelipanche", delta=-500) == 0
