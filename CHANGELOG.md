...
- **`jobd_list` no longer fetches the entire job history on every call (audit L-8).** It issued at most one `state_filter` and pulled the *unfiltered* list to compute counts — the transfer the broker's pagination (v0.5.22) was built to end, still happening on every default call. It now issues one **bounded** request per requested state and takes exact per-state counts from `X-Total-Count` (new `client.list_jobs_with_total`); the default active-state set is **derived** from `JobState`/`TERMINAL_STATES` instead of hand-listed, so a future non-terminal state can't silently vanish from the default view.
...
- **The MCP call log can no longer fail the tool call it logs (audit Q-2).** `_log_call` runs in a `finally` on the hot path with no guard; a read-only HOME or full disk turned every *successful* broker result into an OSError. Filesystem errors are swallowed — telemetry never outranks the payload.
- **The `jobd_events` schema is derived from the real event vocabulary (audit Q-1).** The hand-typed list was stale within a day of shipping — 7 real event types (`job_resurrected` among them) weren't advertised, so agents were told those filter values didn't exist. The description now derives from `models.KNOWN_EVENTS` (itself AST-sweep-tested for completeness), the `source` enum covers `hook`/`mcp` (the old enum *forbade* filtering to events the broker records fine), and parity tests hold both.
