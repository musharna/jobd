...
`~/project-y/.claude/worktrees/embodiment-v8` (a git worktree, which
...
Live incident (2026-06-30): project-y GPU test job submitted from a worktree cwd
...
Broker (A + the refuse-admission/next-job/schema change) runs on **broker-host**
(docker). Worker change (B's cwd check) runs on **all four** workers (laptop,
desktop, broker-host, worker-b), each with its own checkout. Editable install on the
