...
> Branch `feat/submit-cwd-probe`. Multi-host: broker (broker-host, docker) + 4 workers
> (laptop, desktop, broker-host, worker-b). All changes are additive/back-compat, so
...
1. **Broker first (broker-host).** Pull `feat/submit-cwd-probe` (or the merge to the
   deployed line) into the broker-host broker checkout; rebuild/restart the
...
2. **Workers (laptop, desktop, broker-host, worker-b).** Update each host's checkout and
   ...
   `src/` directly. Desktop/broker-host/msi: update their `~/jobd-worker/` (or
...
```bash
cd ~/project-y/.claude/worktrees/<some-worktree>
job submit --project project-y --cwd $(pwd) --gpu --needs cuda-24gb --wait -- \
  bash -lc 'echo HOST=$(hostname); pwd'
```

Expected (new behavior): the job either (a) runs on `laptop` (`HOST=LAPTOP-HOST`) —
