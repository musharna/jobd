...
| `job_cancelled` | The job was cancelled by request or by a dependency cascade. Payload carries `reason` when the canceller gave one (`job cancel --reason`, `jobd_cancel reason`). |
